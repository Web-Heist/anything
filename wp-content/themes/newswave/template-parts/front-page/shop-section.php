<?php 
    $ed_shop_section = newswave_get_option( 'enable_shop_section' );
    $section_title   = newswave_get_option( 'shop_post_title' );
    $section_content = newswave_get_option( 'shop_post_description' );
    $product_type    = newswave_get_option( 'shop_select_product_from' );
    $custom_product  = newswave_get_option( 'select_product_category' );
    $button_lbl      = newswave_get_option( 'shop_post_button_text' );
    $button_link     = newswave_get_option( 'shop_post_button_url' );
    $shop_number_of_column  = newswave_get_option( 'shop_number_of_column' );
    $shop_number_of_product = newswave_get_option( 'shop_number_of_product' );
    if( $ed_shop_section ){
                                    ...
                                    <div class="theme-product-image">
                                        ...
                                        <a href="<?php the_permalink(); ?>" rel="bookmark">
                                            ...
                                            if (has_post_thumbnail()) {
                                                ...
                                                newswave_theme_svg('newswave-shop'); //fallback
                                            }
                                            ?>
                                        </a>
                                        <?php
                                        $stock = get_post_meta(get_the_ID(), '_stock_status', true);
                                        if ($stock == 'outofstock') {
                                            echo '<span class="outofstock-label">' . esc_html__('Sold Out', 'newswave') . '</span>';
                                        } else {
