...
$class = '';
$page_layout = newswave_get_page_layout();
$current_id = '';
if (have_posts()) :
    while (have_posts()) :
        the_post();
...
<?php
global $post;
$newswave_post_layout = esc_html(get_post_meta($current_id, 'newswave_post_layout', true));
$featured_image = "";
if (has_post_thumbnail($post->ID)) {
    $featured_image = wp_get_attachment_image_src(get_post_thumbnail_id($post->ID), 'large');
    $featured_image = isset($featured_image[0]) ? $featured_image[0] : '';
}
if ($newswave_post_layout == "layout-2") { ?>
    <div class="single-featured-banner">
        ...
        <div class="featured-banner-content">
            <div class="wrapper">
                ...
                ?>
                    <div class="newswave-entry-meta">
                        <?php
                        newswave_posted_on();
                        $byline = sprintf(
                            /* translators: %s: post author. */
                            esc_html_x('by %s', 'post author', 'newswave'),
                            '<a class="url fn n" href="' . esc_url(get_author_posts_url($post->post_author)) . '">' . esc_html(get_the_author_meta('nicename', $post->post_author)) . '</a>'
                        );
...
<?php } elseif ($newswave_post_layout == "layout-3"){ ?>
    ...
    <div class="wrapper">
        <div id="primary" class="content-area theme-sticky-component">
            ...
            while (have_posts()) :
                ...
                the_post_navigation(
                    array(
                        'prev_text' => '<span class="nav-subtitle">' . esc_html__('Previous:', 'newswave') . '</span> <span class="nav-title">%title</span>',
                        'next_text' => '<span class="nav-subtitle">' . esc_html__('Next:', 'newswave') . '</span> <span class="nav-title">%title</span>',
                    )
                );
                ...
                if ('post' === get_post_type()) :
                    ...
                    $show_author_info = newswave_get_option('show_author_info');
                    $show_related_posts = newswave_get_option('show_related_posts');
                    $show_author_posts = newswave_get_option('show_author_posts');
...
<?php $show_sticky_article_navigation = newswave_get_option('show_sticky_article_navigation');
    ...
    <div class="sticky-article-navigation">
        <?php if (isset($prev_post->ID)) {
            ...
            <a class="sticky-article-link sticky-article-prev" href="<?php echo esc_url($prev_link); ?>">
                <div class="sticky-article-icon">
                    <?php newswave_theme_svg('arrow-left'); ?>
                </div>
        ...
        if (isset($next_post->ID)) {
            ...
            <a class="sticky-article-link sticky-article-next" href="<?php echo esc_url($next_link); ?>">
                <div class="sticky-article-icon">
                    <?php newswave_theme_svg('arrow-right'); ?>
                </div>
                <article id="post-<?php the_ID(); ?>" <?php post_class('theme-article-post theme-sticky-article'); ?>>
                    <?php if (has_post_thumbnail()) : ?>
                        <div class="entry-image">
