...
if ( ! function_exists( 'newswave_ajax_pagination' ) ) :
    /**
     ...
     */
    function newswave_ajax_pagination($type) {
        global $wp_query; 
        if ( $wp_query->max_num_pages > 1  ) {

            ?>
            <div class="newswave-load-posts-btn-wrapper" data-page="1" data-max-pages="<?php echo esc_attr($wp_query->max_num_pages); ?>" data-load-type="<?php echo esc_attr($type); ?>">
                <hr>
                <a href="#" class="ajax-viewmore-link">
                    <span class="newswave-ajax-loader"></span>
                    <button class="load-btn"><?php _e('Load More', 'newswave') ?></button>
                </a>
            </div>
            <?php
        }
...
if ( ! function_exists( 'newswave_load_posts' ) ) :
    /**
     ...
     *
     */
    function newswave_load_posts() {

        check_ajax_referer( 'newswave-load-posts-nonce', 'nonce' );
        ...
        if($posts->have_posts()):
            ...
            $error = new WP_Error( '500', __('No More Posts','newswave') );
...
add_action( 'wp_ajax_newswave_load_posts', 'newswave_load_posts' );
add_action( 'wp_ajax_nopriv_newswave_load_posts', 'newswave_load_posts' );