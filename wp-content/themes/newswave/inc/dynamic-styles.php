...
/* ------------------------------------ */
if (!function_exists('newswave_hex2rgb')) {
    function newswave_hex2rgb($hex, $array = false)
    {
        $hex = str_replace("#", "", $hex);
        if (strlen($hex) == 3) {
...
if (!function_exists('newswave_get_inline_css')) :
    /**
     * Outputs theme custom CSS.
     *
     * @since 1.0.0
     */
    function newswave_get_inline_css()
    {
        $defaults = newswave_get_default_customizer_values();
        $site_title_text_size = newswave_get_option('site_title_text_size');
        $site_tagline_text_size = newswave_get_option('site_tagline_text_size');
        $banner_bg_color = newswave_get_option('banner_bg_color');
        $banner_text_color = newswave_get_option('banner_text_color');
        $top_bar_bg_color = newswave_get_option('top_bar_bg_color');
        $top_bar_text_color = newswave_get_option('top_bar_text_color');
        $header_bg_color = newswave_get_option('header_bg_color');
        $progressbar_color = newswave_get_option('progressbar_color');
        
        $article_with_separator_bg_color = newswave_get_option('article_with_separator_bg_color');
        $widget_separator_bg_color = newswave_get_option('widget_separator_bg_color');


        $footer_bg_color = newswave_get_option('footer_bg_color');
...
if (!function_exists('newswave_get_dark_mode_inline_css')) :
    /**
     * Outputs theme dark mode custom CSS.
     *
     * @since 1.0.0
     */
    function newswave_get_dark_mode_inline_css()
    {
        $defaults = newswave_get_default_customizer_values();
        $dark_mode_accent_color = newswave_get_option('dark_mode_accent_color');
        $dark_mode_bg_color = newswave_get_option('dark_mode_bg_color');
        $dark_mode_text_color = newswave_get_option('dark_mode_text_color');
        ob_start();
        ?>
    ...
    <?php endif; ?>
        ...
        background-color: <?php echo esc_attr(newswave_hex2rgb($dark_mode_accent_color, 0.03)); ?>;
        }
